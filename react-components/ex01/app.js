...
const arr3 = person.friends.map(({ name, age }) => {
  ...
  return `Diferenta dintre ${name} si ${person.name} este ${diff}`;
});
...
console.warn(
  ...
  array care contine cuvintele cu prima si ultima litera mari. `,
);
...
console.warn(
  `Folosind metoda map pe arrayul friends, returneaza un array care contine numele
   inversat al prietenilor pe fiecare pozitie (exemplu: Stevenson Steven)
  `,
);
...
console.warn(
  `Folosind metoda map pe arrayul friends, returneaza un array care contine pe
  fiecare pozitie diferenta dintre lungimea totala al numelui complet (fara spatii) si varsta
  prietenului de pe iteratie
  `,
);

const arr9 = person.friends.map(({ name, age }) => {
